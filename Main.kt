...
fun main() {
    ...
    arena.onTimeProgress(25){
        if (game.nave.tiroy > 0){
            ...
            println("${arena.positionShot(game.nave.tiroy)}")
        }
...
fun Canvas.drawAll (game : Game) :  Game{
    ...
    game.enemy.map{ Enemy(it.x,MoveEnemy(it),it.delta) }
...
fun Canvas.createEnemy (game : Game) : List<Enemy> { return  game.enemy + Enemy(Random.nextInt(0,700),0, Random.nextInt(2,24)) }


fun Canvas.MoveEnemy (enemy : Enemy) : Int{
