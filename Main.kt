...
fun main() {
    ...
    onStart {
        arena.onMouseMove { mouse : MouseEvent ->
            game = Game(Spaceship(mouse.x - 23, game.nave.tirox, game.nave.tiroy),game.enemy) //Altera o posicionamento segundo o eixo do x da nave
            arena.drawAll(game)
        }
...
fun Canvas.interception (game : Game) : Game{
    var xi = game.nave.tirox - 2
    var xf = game.nave.tirox +2
    var y = game.nave.tiroy

    return Game(game.nave, game.enemy.mapNotNull { if (it.x in xi..xf && it.y in y..y-7) null else it})
